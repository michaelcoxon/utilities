...
export default class DateFormatter implements IFormatter<Date>
{
    ...
    private static readonly TOKEN_TO_STRING_DELEGATES: Record<string, (d: Date, config: IDateFormatterConfiguration) => string> = {
    ...
    public format(subject: Date, format: string): string
    {
        ...
        const result = format.replace(matcher, (token: string) =>
        {
            ...
            {
                return DateFormatter.TOKEN_TO_STRING_DELEGATES[token](subject, this.#config);
            }
            return token;
        });
